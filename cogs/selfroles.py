...
class SelfRoles:
    ...
    async def selfrole(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            entity=self.bot.get_command("selfrole")
    ...
    async def add(self, ctx, *, role: discord.Role):
        ...
        dataIO.save_json(self.file_path, self.data)
        await ctx.send(f"Added **{role.name}** to the self roles list {self.bot.get_emoji(452709464546476034)}")
    ...
    async def remove(self, ctx, *, role: discord.Role):
        ...
        dataIO.save_json(self.file_path, self.data)
        await ctx.send(f"Removed **{role.name}** from the self roles list {self.bot.get_emoji(452709464546476034)}")
    ...
    async def reset(self, ctx):
        ...
        self.data[str(server.id)] = {}
        dataIO.save_json(self.file_path, self.data)
        await ctx.send(f"All self roles have been deleted {self.bot.get_emoji(452709464546476034)}")
    ...
    @commands.command()
    async def myrole(self, ctx, *, role: discord.Role):
