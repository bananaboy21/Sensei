...
class giveaway:
    def __init__(self, bot):
        self.bot = bot
        self._giveaway_task = asyncio.get_event_loop().create_task(self.check_giveaway())
    ...
    async def giveaway(self, ctx):
        ...
        if ctx.invoked_subcommand is None:
            entity=self.bot.get_command("giveaway")
    ...
    async def delete(self, ctx, id: str):
        try:
            message = await self.bot.get_channel(
                int(self._giveaway[str(ctx.guild.id)]["giveaways"][id][0]["channel"])).get_message(
                int(self._giveaway[str(ctx.guild.id)]["giveaways"][id][0]["message"]))
        except:
    ...
    async def setup(self, ctx):
        ...
        def check_time(m):
            if m.content.lower() == "cancel" and m.channel == ctx.channel and m.author == ctx.author:
                return True
            if m.channel == ctx.channel and m.author == ctx.author and m.content.isdigit() and int(
                    m.content) >= 30 and int(m.content) <= 31556926:
                return True
        ...
        await ctx.send(
            "What channel would you like me to start this giveaway? Type \"cancel\" at anytime to cancel the creation (Respond below)")
        try:
        ...
        s = discord.Embed(title=title.content,
                          description="Enter by reacting with :tada:\n\nThis giveaway is for **{}**\nDuration: **{}**\nWinners: **{}**".format(
                              item.content, await self.giveaway_time(starttime, endtime), int(winners.content)),
                          timestamp=datetime.fromtimestamp(datetime.utcnow().timestamp() + int(time2.content)))
                ...
                for x in list(self._giveaway[serverid]["giveaways"])[:len(list(self._giveaway[serverid]["giveaways"]))]:
                    for number in self._giveaway[serverid]["giveaways"][x]:
                        if number["endtime"] <= datetime.utcnow().timestamp():
                            try:
                                message = await self.bot.get_channel(int(number["channel"])).get_message(
                                    int(number["message"]))
                                reaction = [x for x in message.reactions if x.emoji == "🎉"][0]
                                try:
                                    winner = random.sample(
                                        [x for x in await reaction.users().flatten() if x != self.bot.user],
                                        k=number["winners"])
                                except:
                                    winner = random.sample(
                                        [x for x in await reaction.users().flatten() if x != self.bot.user],
                                        k=len([x for x in await reaction.users().flatten() if x != self.bot.user]))
                                if winner == []:
                                    await self.bot.get_channel(int(number["channel"])).send(
                                        "No one entered the giveaway :no_entry:")
                                    await message.delete()
                                    del self._giveaway[serverid]["giveaways"][x]
                                    dataIO.save_json(self._giveaway_file, self._giveaway)
                                    return
                                s = discord.Embed(title=number["title"], description="**{}** has won **{}**".format(
                                    ", ".join([str(x) for x in winner]), number["item"]))
                                s.set_footer(text="Giveaway Ended")
                                await message.edit(embed=s)
                                await self.bot.get_channel(int(number["channel"])).send(
                                    "Congrats {}, you won the giveaway for **{}**".format(
                                        ", ".join([x.mention for x in winner]), number["item"]))
                                del self._giveaway[serverid]["giveaways"][x]
                                dataIO.save_json(self._giveaway_file, self._giveaway)
                            except:
                                pass
